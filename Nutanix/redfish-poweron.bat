...
set "SCRIPT_DIR=%~dp0"
set "LOG_FILE=%SCRIPT_DIR%nutanix-auto-start.log"
set "TMP_DIR=%TEMP%\ntnx_rf_%RANDOM%%RANDOM%"
mkdir "%TMP_DIR%" >nul 2>&1

call :log "=== Nutanix Cluster Startup By ZenithComp ==="
    ...
    for /l %%I in (1,1,%HOST_COUNT%) do (
        if "!TRACKER_%%I!"=="booting" (
            ...
            if /i "!PS_%%I!"=="On" (
                ...
                call :log "Host:!HOST_%%I! ONLINE - startup complete"
            )
        )
    )
goto monitor_loop
...
if "!CPS!"=="Error" (
    call :log "Host:!IP! State:API Error - check credentials/network"
...
powershell -NoProfile -Command ^
    "$u='!BMC_USER!';$p='!BMC_PASS!';$b=@{UserName=$u;Password=$p}|ConvertTo-Json -Compress;[IO.File]::WriteAllText('!SREQ!',$b)"
...
for /f "usebackq delims=" %%B in (`powershell -NoProfile -Command ^
    "[Convert]::ToBase64String([Text.Encoding]::ASCII.GetBytes('!BMC_USER!:!BMC_PASS!'))"`) do (
    set "B64_%IDX%=%%B"
)
